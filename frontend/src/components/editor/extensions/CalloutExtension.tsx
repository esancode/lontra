...
const CalloutComponent: React.FC<CalloutProps> = ({ node, updateAttributes, selected }) => {
  const { type } = node.attrs;
  const [showOptions, setShowOptions] = useState(false);

  const calloutTypes = [
    { type: 'info', icon: <Info size={20} />, label: 'Informação', color: 'bg-blue-500/10 border-blue-500/50 text-blue-400' },
    { type: 'warning', icon: <AlertTriangle size={20} />, label: 'Aviso', color: 'bg-yellow-500/10 border-yellow-500/50 text-yellow-400' },
    { type: 'error', icon: <AlertOctagon size={20} />, label: 'Erro', color: 'bg-red-500/10 border-red-500/50 text-red-400' },
    { type: 'success', icon: <CheckCircle2 size={20} />, label: 'Sucesso', color: 'bg-green-500/10 border-green-500/50 text-green-400' },
  ];
  ...
  return (
    ...
    >
      <div className={`flex gap-3 p-4 rounded-[var(--radius-md)] border-l-4 transition-colors ${currentConfig.color}`}>
        <div 
          className="callout-icon shrink-0 select-none cursor-pointer relative flex items-start pt-0.5"
          onClick={() => setShowOptions(!showOptions)}
          title="Mudar tipo de aviso"
        >
          <span className="flex items-center justify-center">{currentConfig.icon}</span>
