...
const BookmarkComponent: React.FC<BookmarkProps> = ({ node, updateAttributes, selected }) => {
  ...
  return (
    <NodeViewWrapper 
      className={`bookmark-block flex items-stretch border border-[var(--border-default)] rounded-[var(--radius-md)] overflow-hidden my-4 hover:bg-[var(--bg-tertiary)] cursor-pointer transition-colors max-w-full group ${selected ? 'ring-2 ring-[var(--accent-blue)]' : ''}`} 
