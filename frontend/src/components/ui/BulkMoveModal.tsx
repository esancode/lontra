...
const BulkMoveModal: React.FC<BulkMoveModalProps> = ({ 
  ...
  const { boxes, notes, moveNote, moveBox } = useAppStore();
  ...
  const filteredBoxes = boxes.filter(box => {
    // Não permitir mover para dentro de caixas que estão selecionadas
    if (selectedIds.has(box._id)) return false;
    if (!box.name.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });

  const handleMove = async (targetId: string | null) => {
    // process in series or parallel, array of promises
    const promises = Array.from(selectedIds).map(id => {
      if (notes.find(n => n._id === id)) {
        return moveNote(id, targetId);
      } else if (boxes.find(b => b._id === id)) {
        return moveBox(id, targetId);
      }
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-default)]">
          <div>
            ...
            <p className="text-[var(--text-muted)] text-xs mt-0.5 truncate max-w-[280px]">Mover {selectedIds.size} itens</p>
          </div>
