...
import { SearchIcon, FileIcon, PlusIcon, FolderPlusIcon, MoonIcon } from 'lucide-react';
import { useNavigate } from 'react-router';
// @ts-ignore
import FlexSearch from 'flexsearch';
import { extractTextFromJSON } from '../../utils/editorUtils';

const CommandPalette: React.FC = () => {
    const { commandPaletteOpen, toggleCommandPalette, notes, boxes, createNote, createBox } = useAppStore();
    ...
    // UseMemo para indexar notas no FlexSearch uma vez na montagem/atualização
    const flexIndex = React.useMemo(() => {
        // @ts-ignore
        ...
        notes.forEach(note => {
            idx.add({
                id: note._id,
                title: note.title,
                content: extractTextFromJSON(note.content),
    ...
    // Calcular os filteredNotes via flexsearch se tiver query
    let filteredNotes = notes.slice(0, 5);
    
    if (searchQuery.trim()) {
        const cleanQuery = searchQuery.replace(/#/g, '');
        const results = flexIndex.search(cleanQuery, 10);
        // Os resultados vêm como array de fields: [{ field: 'title', result: [id1, id2] }, ...]
        const resultSet = new Set<string>();
        results.forEach((resItem: any) => {
            resItem.result.forEach((id: string) => resultSet.add(id));
        });
        
        filteredNotes = notes.filter(n => resultSet.has(n._id)).slice(0, 5);
    }
